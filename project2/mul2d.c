...
void multiply(int n, float a[n][n], float b[n][n], float result[n][n] ) {
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            result[i][j] = 0;
            for (int k = 0; k < n; k++)
...
void block_multiply(int n, float a[n][n], float b[n][n], float result[][n], int block_size) {
...
int main(int argc, char* argv[]) {
    ...
    float (*result)[n] = malloc(sizeof(float[n][n]));
